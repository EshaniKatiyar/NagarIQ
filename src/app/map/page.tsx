    ...
    <div className="h-[calc(100vh-4rem)] flex relative">
      ...
      {selectedIssue && (
        <div className="absolute top-0 right-0 h-full w-80 glass shadow-2xl flex flex-col overflow-y-auto z-[1000] animate-slide-up">
          ...
          {briefResult && briefResult.briefId && (
            <div className="m-4 bg-green-50 border border-green-200 rounded-xl p-4">
              ...
              <p className="text-xs text-green-700 mb-3">Routed to {briefResult.brief?.department || selectedIssue?.department || 'the relevant department'}</p>
              <Link href={`/brief/${briefResult.briefId}`} className="w-full flex items-center justify-center gap-1.5 py-2 rounded-xl text-sm font-medium bg-amber-500 text-white hover:bg-amber-600 transition-colors">
                <ExternalLink className="w-3.5 h-3.5" /> View Civic Brief
              </Link>
            </div>
          )}
